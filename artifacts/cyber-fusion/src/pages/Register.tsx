...
import { TerminalSquare, Lock, Mail, Building2, User, Key, AlertCircle, Loader2, CheckCircle2 } from "lucide-react";
        ...
        <div className="flex items-center gap-3 mb-6">
          {[1, 2].map(s => (
            <div key={s} className={cn("flex items-center gap-2 flex-1", s < step ? "opacity-60" : "")}>
              <div className={cn("h-6 w-6 flex items-center justify-center text-xs font-mono font-bold border shrink-0", s === step ? "border-primary text-primary bg-primary/10" : s < step ? "border-primary/40 bg-primary/5" : "border-border text-muted-foreground")}>
          ...
          {step === 2 && (
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {TIERS.map(t => (
                    <button
                      ...
                      className={cn("border p-2.5 text-left transition-colors", form.tier === t.id ? "border-primary bg-primary/10 text-primary" : "border-border text-muted-foreground hover:border-muted-foreground")}
                    >
              ...
              <div className="flex gap-3">
                ...
                <button type="submit" disabled={loading} className={cn("flex-1 flex items-center justify-center gap-2 py-2.5 text-sm font-mono font-bold tracking-widest bg-primary text-primary-foreground hover:opacity-90 disabled:opacity-50")}>
