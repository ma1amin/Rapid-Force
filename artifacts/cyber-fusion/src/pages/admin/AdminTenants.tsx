...
function CopyKey({ licenseKey }: { licenseKey: string }) {
  ...
  return (
    <button onClick={async (e) => { e.stopPropagation(); await navigator.clipboard.writeText(licenseKey); setCopied(true); setTimeout(() => setCopied(false), 2000); }}
...
export default function AdminTenants() {
  ...
  const [bulkTier, setBulkTier] = useState<string>("starter");
  ...
  const runBulkAction = async (action: "suspend" | "activate" | "tier") => {
    if (selectedIds.size === 0) return;
    setBulkLoading(true);
    try {
      const body: any = { ids: Array.from(selectedIds), action };
      if (action === "tier") body.tier = bulkTier;
  ...
  return (
    <div className="space-y-5">
      ...
      {selectedIds.size > 0 && (
        <div className="flex items-center gap-3 bg-orange-500/10 border border-orange-500/30 px-4 py-2.5">
          ...
          <div className="relative">
            ...
            {tierDropOpen && (
              <div className="absolute right-0 top-full mt-1 bg-card border border-border z-20 min-w-[120px]">
                {TIERS.map((t) => (
                  <button key={t} onClick={() => { setBulkTier(t); runBulkAction("tier"); }}
      ...
      <div className="bg-card border border-border overflow-hidden">
        <div className="grid grid-cols-[auto_1fr_auto_auto_auto_auto_auto] gap-4 px-5 py-2.5 border-b border-border text-[10px] font-mono text-muted-foreground tracking-widest items-center">
        ...
        {loading ? (
          <div className="space-y-px">
            {[...Array(5)].map((_, i) => <div key={i} className="h-14 bg-muted/30 animate-pulse border-b border-border" />)}
          </div>
        ) : filtered.length === 0 ? (
          ...
          <div className="divide-y divide-border">
            {filtered.map((t) => (
              <div key={t.id}
                className={cn("grid grid-cols-[auto_1fr_auto_auto_auto_auto_auto] gap-4 items-center px-5 py-3.5 transition-colors",
                ...
                <div className="flex items-center gap-1 text-xs font-mono text-muted-foreground">
                  <Users className="h-3.5 w-3.5" />{t.userCount}
                </div>
                ...
                <div className="flex items-center gap-2">
                  <button onClick={() => toggleActive(t)} disabled={togglingId === t.id}
                    className={cn("text-xs font-mono border px-2 py-1 transition-colors disabled:opacity-40",
                      t.isActive
