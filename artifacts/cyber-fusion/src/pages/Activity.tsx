...
export default function Activity() {
  ...
  const { data: activity, isLoading, isFetching, dataUpdatedAt } = useListActivity(
    { limit: 100 },
    { query: { queryKey: getListActivityQueryKey({ limit: 100 }), refetchInterval: REFETCH_MS } }
  );

  const handleRefresh = () => {
    qc.invalidateQueries({ queryKey: getListActivityQueryKey({ limit: 100 }) });
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between">
        ...
        <div className="flex items-center gap-3">
          <div className="text-right">
            <div className="text-xs font-mono text-muted-foreground">LAST SYNC</div>
            <div className="text-xs font-mono text-primary">{lastUpdated}</div>
          </div>
          <button
      ...
      {/* Stats bar */}
      <div className="flex gap-6 text-xs font-mono text-muted-foreground border-b border-border pb-3">
        <span><ActivitySquare className="h-3 w-3 inline mr-1" />TOTAL: <span className="text-foreground">{activity?.length ?? 0}</span></span>
        <span>THREATS: <span className="text-destructive">{activity?.filter(e => e.type.startsWith("threat")).length ?? 0}</span></span>
        <span>MISSIONS: <span className="text-primary">{activity?.filter(e => e.type.startsWith("mission")).length ?? 0}</span></span>
        <span>SPRINTS: <span className="text-accent">{activity?.filter(e => e.type.startsWith("sprint")).length ?? 0}</span></span>
        <span>AGENTS: <span className="text-primary">{activity?.filter(e => e.type.startsWith("agent")).length ?? 0}</span></span>
        <span className="ml-auto text-muted-foreground/50">AUTO-REFRESH 15s</span>
      </div>
