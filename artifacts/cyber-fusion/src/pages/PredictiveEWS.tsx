    ...
    <div className="p-6 space-y-6">
      ...
      {critical > 0 && (
        <div className="bg-red-950/40 border border-red-500/40 rounded-lg px-4 py-3 flex items-center gap-3">
          ...
          <div className="text-xs font-mono text-red-300 flex-1">
          ...
          <Button size="sm" onClick={() => setEscalateOpen(true)}
            className="bg-red-500/20 border border-red-500/40 text-red-300 hover:bg-red-500/30 font-mono text-xs h-7 gap-1.5 flex-shrink-0">
            ...
            <div className="space-y-4">
              ...
              <div className="flex flex-col gap-2">
                ...
                {selected.severity === "critical" && (
                  <Button className="w-full bg-red-500/10 border border-red-500/30 text-red-400 hover:bg-red-500/20 font-mono text-xs gap-2"
      ...
      <Dialog open={escalateOpen} onOpenChange={setEscalateOpen}>
        <DialogContent className="bg-slate-900 border border-red-500/30 text-slate-100 max-w-lg font-mono">
          <DialogHeader>
            <DialogTitle className="text-red-400 tracking-widest text-sm font-mono flex items-center gap-2">
              <AlertTriangle className="w-4 h-4" /> ESCALATE CRITICAL SIGNALS
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-4 pt-1">
            <div className="text-xs text-slate-400 font-mono bg-red-950/30 border border-red-500/20 rounded p-3">
              Escalating <span className="text-red-300 font-bold">{critical} critical signal{critical > 1 ? "s" : ""}</span> will
              immediately notify the selected stakeholders via secure channel and create a formal incident record.
            </div>

            {/* Escalation targets */}
            <div className="space-y-1.5">
              <Label className="text-xs text-slate-400 tracking-wider">NOTIFY STAKEHOLDERS</Label>
              <div className="space-y-2">
                {ESCALATION_TARGETS.map(t => (
                  <div key={t.id} className={cn(
                    ...
                    />
                    <t.icon className={cn("w-3.5 h-3.5 flex-shrink-0", escalateTargets[t.id] ? "text-red-400" : "text-slate-500")} />
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <span className={cn("text-xs font-mono font-semibold", escalateTargets[t.id] ? "text-slate-200" : "text-slate-500")}>
                          {t.label}
                        </span>
                        {t.required && (
                          <span className="text-[9px] font-mono text-red-500 border border-red-500/30 px-1 py-0.5 rounded">REQUIRED</span>
                        )}
                      </div>
                      <div className="text-[10px] text-slate-500 font-mono">{t.description}</div>
                    </div>
                  </div>
                ))}
              </div>
            ...
            {/* Notes */}
            <div className="space-y-1.5">
              <Label className="text-xs text-slate-400 tracking-wider">ESCALATION NOTES (OPTIONAL)</Label>
            ...
            {/* Summary */}
            <div className="text-xs font-mono text-slate-400 bg-slate-800/50 border border-slate-700/50 rounded p-3">
              <span className="text-slate-300">Will notify:</span>{" "}
            ...
            <div className="flex gap-3">
              ...
              <Button onClick={handleEscalate} disabled={selectedTargets.length === 0}
                className="flex-1 bg-red-500/20 border border-red-500/40 text-red-300 hover:bg-red-500/30 font-mono text-xs h-9 gap-2 disabled:opacity-40">
