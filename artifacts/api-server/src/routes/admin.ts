...
import { db, tenantsTable, usersTable, moduleLicensesTable } from "@workspace/db";
...
router.post("/admin/auth/login", async (req, res) => {
  ...
  if (!adminEmail || !adminPassword) {
    res.status(503).json({ error: "Platform admin not configured" });
    return;
  }
  ...
  try {
    const tenants = await db
      .select({
        ...
        tier: tenantsTable.tier,
        licenseKey: tenantsTable.licenseKey,
  ...
  try {
    const [[tenant], users, modules] = await Promise.all([
      ...
      db.select({
        ...
        role: usersTable.role, isActive: usersTable.isActive, lastLoginAt: usersTable.lastLoginAt, createdAt: usersTable.createdAt,
